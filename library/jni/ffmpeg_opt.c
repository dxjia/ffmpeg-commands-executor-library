...
void show_help_default(const char *opt, const char *arg)
{
    ...
    if (opt && *opt) {
        if (!strcmp(opt, "long"))
            ...
            RECORD(NULL, AV_LOG_ERROR, "Unknown help option '%s'.\n", opt);
    }

    show_usage();

    RECORD_P("Getting help:\n"
...
{
    RECORD(NULL, AV_LOG_INFO, "Hyper fast Audio and Video encoder\n");
    RECORD(NULL, AV_LOG_INFO, "usage: %s [options] [[infile options] -i infile]... {[outfile options] outfile}...\n", program_name);
    RECORD(NULL, AV_LOG_INFO, "\n");
}
